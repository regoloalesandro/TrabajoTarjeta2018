...
class Medioboleto extends  Tarjeta {
	...
	public function reducirSaldo($valor){
		...
		if($this->ultviaje['year'] == getdate( $this->tiempo )['year'] && $this->ultviaje['mon'] == getdate( $this->tiempo )['mon'] && $this->ultviaje['mday'] == getdate( $this->tiempo )['mday']){
			//Si fue realizado en la misma "hora"
			if($this->ultviaje['hours'] == getdate( $this->tiempo )['hours']){
				if($this->ultviaje['minutes']+5 >= getdate( $this->tiempo )['minutes'] ){
					return false;
				}
			}
			//Si hubo un cambio de hora entre los 2 viajes. Ej ultviaje a las 14:57 y se intenta viajar a las 15:01
			elseif($this->ultviaje['hours'] == getdate( $this->tiempo )['hours']-1 ){
				if($this->ultviaje['minutes']+5 >= getdate( $this->tiempo )['minutes']+60){
					return false;
				}
			}
		...
		$this->ultviaje = getdate( $this->tiempo );
